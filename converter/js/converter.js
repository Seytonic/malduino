...
function convert(){
	...
	if(altCodes.checked === true) script = convertAltCodes(script);
	if(deleteCommentsSetting.checked === true)script = deleteComments(script);
	script = deleteBreaks(script);
	if(optimizeSetting.checked === true) script = optimize(script);
	if(delay.checked === true) script = addDelay(script);
	if(autoCorrectSetting) script = autocorrect(script);
